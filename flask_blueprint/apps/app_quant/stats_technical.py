...
def std(se, N=None, ddof=0):
    """ std = np.sqrt(np.mean(np.abs(x - x.mean())**2))
    ...
    # X! df = df.join(pd.Series(df['Close'].rolling(n, min_periods=n).std(), name='STD_' + str(n)))
    return se.rolling(N, min_periods=N).std(ddof=ddof)


def vol_hist(se, N, per_annual=252):
    ''' Vol ~ std
        Implied volatility looks forward in time, being derived from the market
        price of a market-traded derivative, in particular, an option.
        Historical volatility looks only at past returns

    annualized standard deviation of returns:

    R_daily = ln(C_n/C_n-1) for every rolling N=21 or (C_n/C_n-1) - 1
    R_daily_mu = (Sum R_i)/n
    var_daily = (Sum(R_i - R_mu)^2)/(n-1)
    vol_std_daily = sqrt(var)

    vol_T = vol_annual * sqrt(T) :
    vol_annual = vol_daily * sqrt(252)

    vol_month  = vol_annual * sqrt(1/12)
               = vol_daily * sqrt(252/12)
    '''
    pass
