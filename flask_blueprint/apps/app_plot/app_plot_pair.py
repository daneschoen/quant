...
TICKER_PATH = "~/projects/fintech/data_in/sym_equity_ticker.csv"

if os.getcwd()[:6] == '/Users':
    TICKER_PATH = "~/Agape/development" + TICKER_PATH[1:]

# todo: or better yet, cache in mongodb
#       import full sp500_lst
def import_data():
    df_stk = import_stk_dct_df(src='pth_equity_sp500', sym_lst=['aapl','goog','nflx'])
    df_fut = import_fut_dct_df()
    df_crp = import_crypto_dct_df()
...
#     # eg, 'stk_aapl-crp_btc'
#     pair_name = asset_0 + '_' + sym_0 + '_' + asset_1 + '_' + sym_1
...
def graph_scatter(asset_0, asset_1, sym_0, sym_1, relayoutData, n_clicks, state):
    ...
    x_all, y_all, x_per, y_per = transform_pctchange_clean_standardize_sample_pd(x_all_raw, y_all_raw, dtrange[0], dtrange[1])
    ...
    model, reg_model = reg_ols_sm(y_per, x_per)
    x_reg = x_per
...
def graph_3d_surface_reg(asset_0, asset_1, sym_0, sym_1):
    ...
    x_all = transform_pctchange_clean_standardize_pd(x_all_raw)
    y_all = transform_pctchange_clean_standardize_pd(y_all_raw)

    x, y, z = calc_matrix_reg(x_all, y_all, PER_CORR)
