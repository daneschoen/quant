...
app_stream_train = dash.Dash(server=app, url_base_pathname='/_' + PLOT_ROUTES['stream_model_training'])

app_stream_train.title = "TensorFlow - Real-time"

css_lst = [
    "https://cdnjs.cloudflare.com/ajax/libs/normalize/7.0.0/normalize.min.css",  # Normalize the CSS
    "https://fonts.googleapis.com/css?family=Open+Sans|Roboto",  # Fonts
    "https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css",
    "/static/css/dash_banner.css",
    "/static/css/dash_stream.css",
]
...
@app_stream_train.server.route('/static/<path:path>')
def static_file_stream_train(path):
    return app_stream_train.send_from_directory('/srv/', path)
...
def div_graph(name):
    ...
    return html.Div([
        html.Div(
            id='div-{name}-graph'.format(name=name),
        ...
        html.Div([
            html.Div([
                ...
                dcc.Checklist(
                    ...
                    values=[],
                    id='checklist-smoothing-options-{name}'.format(name=name)
                )
            ],
            ...
            html.Div([
                dcc.Slider(
                    ...
                    value=0.6,
                    updatemode='drag',
                    id='slider-smoothing-{name}'.format(name=name)
                )
            ],
            ...
            html.Div([
                ...
                dcc.RadioItems(
                    ...
                    value='overlap',
                    id='radio-display-mode-{name}'.format(name=name)
                ),

                html.Div(id='div-current-{name}-value'.format(name=name))
            ]),
        ],
...
app_stream_train.layout = html.Div([

    html.Link(rel='stylesheet', href=css_lst[0]),
    html.Link(rel='stylesheet', href=css_lst[1]),
    html.Link(rel='stylesheet', href=css_lst[2]),
    html.Link(rel='stylesheet', href=css_lst[3]),

    # Banner display
    html.Div([
        html.H2(
            'TensorFlow - Real-time Model Training',
            id='title'
        ),
...
demo_callbacks(app_stream_train, demo_mode)


@app_stream_train.callback(Output('interval-log-update', 'interval'),
...
if not demo_mode:
    @app_stream_train.callback(Output('run-log-storage', 'children'),
...
@app_stream_train.callback(Output('div-step-display', 'children'),
              [Input('run-log-storage', 'children')])
def update_div_step_display(run_log_json):
    if run_log_json:
        run_log_df = pd.read_json(run_log_json, orient='split')
        # return html.H6("Step: {run_log_df['step'].iloc[-1]}", style={'margin-top': '3px'})
        return html.H6("Step: " + run_log_df['step'].iloc[-1], style={'margin-top': '3px'})


@app_stream_train.callback(Output('div-accuracy-graph', 'children'),
              [Input('run-log-storage', 'children'),
...
@app_stream_train.callback(Output('div-cross-entropy-graph', 'children'),
              [Input('run-log-storage', 'children'),
...
@app_stream_train.callback(Output('div-current-accuracy-value', 'children'),
              [Input('run-log-storage', 'children')])
def update_div_current_accuracy_value(run_log_json):
    if run_log_json:
        ...
        return [
            ...
            #html.Div("Training: {run_log_df['train accuracy'].iloc[-1]:.4f}"),
            #html.Div("Validation: {run_log_df['val accuracy'].iloc[-1]:.4f}")
            html.Div("Training: {:.4f}".format(run_log_df['train accuracy'].iloc[-1])),
            html.Div("Validation: {:.4f}".format(run_log_df['val accuracy'].iloc[-1]))
        ]


@app_stream_train.callback(Output('div-current-cross-entropy-value', 'children'),
              [Input('run-log-storage', 'children')])
def update_div_current_cross_entropy_value(run_log_json):
    if run_log_json:
        ...
        return [
            ...
            # html.Div("Training: {run_log_df['train cross entropy'].iloc[-1]:.4f}"),
            # html.Div("Validation: {run_log_df['val cross entropy'].iloc[-1]:.4f}")
            html.Div("Training: {:.4f}".format(run_log_df['train cross entropy'].iloc[-1])),
            html.Div("Validation: {:.4f}".format(run_log_df['val cross entropy'].iloc[-1]))
        ]
