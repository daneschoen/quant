...
# ==============================================================================
# app_basic = dash.Dash(server=app)
app_basic = dash.Dash(server=app, url_base_pathname='/basic/')
# app_basic = dash.Dash(__name__)
app_basic.config.supress_callback_exceptions = True

app_basic.css.config.serve_locally = True
app_basic.scripts.config.serve_locally = True

app_basic.css.append_css({
    "external_url": "/static/css/dash.css"  #"https://codepen.io/chriddyp/pen/bWLwgP.css"
})
# app_basic.server.static_folder = 'static'  # if you run app_basic.py from 'root-dir-name' you don't need to specify.
...
app_basic.layout = html.Div(style={'width': '600'}, children=[
    #html.Div(id='content'),
    ...
    html.Div(dcc.Graph(id='graph_3_0', config={'displayModeBar': False})),
    html.Div(id='graph_4_0')
...
@app_basic.callback(Output('amount', 'children'),
              [Input('hours', 'value'), Input('rate', 'value')])
def compute_amount(hours, rate):
    return float(hours) * float(rate)

@app_basic.callback(Output('amount-per-week', 'children'),
              [Input('amount', 'children')])
def compute_week(amount):
    return float(amount) * 7


# @app_basic.callback(Output('graph_2_0', 'children'),
#                     [Input('hola', 'id')])
...
@app_basic.callback(Output('graph_3_0', 'figure'),
...
@app_basic.callback(Output('graph_4_0', 'children'),
             [Input('dropdown_ticker', 'value')])
def foo2(_):
    dcc_graph = dcc.Graph(id='foo',
        figure={
...
# $ FLASK_APP=a_basic_dash_graph.py flask run
# OR:
# if __name__ == '__main__':
    #pass
#   app.run(debug=True)
    #app_basic.run_server(debug=True)
