...
app_stream_wind = dash.Dash(server=app, url_base_pathname='/_' + PLOT_ROUTES['stream_wind'])


if 'DYNO' in os.environ:
    app_stream_wind.scripts.append_script({
...
@app_stream_wind.server.route('/static/<path:path>')
def static_file_stream_wind(path):
    """
    ...
    """
    return app_stream_wind.send_from_directory('/srv/', path)


app_stream_wind.layout = html.Div([
    ...
    html.Link(
      rel='stylesheet',
      href='/static/css/dash-wind-streaming.css'
    ),
    html.Link(
      rel='stylesheet',
      href='https://fonts.googleapis.com/css?family=Raleway:400,400i,700,700i'
    ),
    html.Link(
      rel='stylesheet',
    ...
    html.Div([
        html.H3("Wind Speed Streaming Real-Time"),
        #html.Img(src=""),
    ], className='banner'),
    html.Div([
        html.Div([
            html.H4("WIND SPEED (mph)")
        ], className='Title'),
        html.Div([
...
@app_stream_wind.callback(Output('wind-speed', 'figure'), [Input('wind-speed-update', 'n_intervals')])
def gen_wind_speed(interval):
    ...
    con = sqlite3.connect("./apps/app_plot/streaming_wind/Data/wind-data.db")   # "./Data/wind-data.db"
...
@app_stream_wind.callback(Output('wind-direction', 'figure'), [Input('wind-speed-update', 'n_intervals')])
def gen_wind_direction(interval):
    ...
    total_time = (hour * 3600) + (minute * 60) + (sec)
    # print("\n\n\n" + os.getcwd() + "\n\n\n")  ==>  ~/projects/fintech/flask_blueprint
    con = sqlite3.connect("./apps/app_plot/streaming_wind/Data/wind-data.db")   # "./Data/wind-data.db"
...
@app_stream_wind.callback(Output('wind-histogram', 'figure'),
...
@app_stream_wind.callback(Output('bin-auto', 'values'), [Input('bin-slider', 'value')],
              [State('wind-speed', 'figure')],
...
@app_stream_wind.callback(Output('bin-size', 'children'), [Input('bin-auto', 'values')],
              [State('bin-slider', 'value')],
              [])
def deselect_auto2(autoValue, sliderValue):
