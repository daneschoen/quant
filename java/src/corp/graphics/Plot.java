...
import program.stats.CubicSpline;
    ...
    protected int[] pointOpt = null;    // point plotting option for each curve
    	...
    	// Calculate axis offset value
   	    public static double offset(double low, double high){
        	...
        	double sh = MathFns.sign(high);
        	double sl = MathFns.sign(low);
        	double offset=0.0D;
        	int eh=0, ed=0;

        	if(sh == sl){
            		ed=(int)Math.floor(MathFns.log10(diff));
            		if(sh==1){
                		eh=(int)Math.floor(MathFns.log10(high));
                		if(eh-ed>1)offset = Math.floor(low*Math.pow(10, -ed))*Math.pow(10,ed);
            		}
            		else{
                		eh=(int)Math.floor(MathFns.log10(Math.abs(low)));
    	...
    	public static int ticks(double low, double high, double[] tickval, String[] tickchar){
        	...
        	switch(dirn){
        	    case 1:     ttickval = new double[nticks];
        	                tickval[0]=MathFns.truncate(low, trunc[range]);
        	                for(int i=1; i<nticks; i++){
            		            tickval[i]  = MathFns.truncate(tickval[i-1]+inc, trunc[range]);
        	                }
        	                break;
        	    case -1:    ttickval = new double[nticks];
        	                ttickval[0]=MathFns.truncate(high, trunc[range]);
        	                for(int i=1; i<nticks; i++){
            		            ttickval[i]  = MathFns.truncate(ttickval[i-1]-inc, trunc[range]);
        	                }
        	                ttickval = MathFns.reverseArray(ttickval);
        	                for(int i=0; i<nticks; i++)tickval[i] = ttickval[i];
        	                break;
        	    case 2:     npExtra = (int)Math.ceil(-low/inc);
        	                nticks += npExtra;
        	                ttickval = new double[nticks];
         	                tickval[0]=MathFns.truncate(-npExtra*inc, trunc[range]);
        	                for(int i=1; i<nticks; i++){
            		            tickval[i]  = MathFns.truncate(tickval[i-1]+inc, trunc[range]);
        	                }
           	                break;
           	    case -2:    npExtra = (int)Math.ceil(high/inc);
        	                nticks += npExtra;
        	                ttickval = new double[nticks];
        	                ttickval[0]=MathFns.truncate(npExtra*inc, trunc[range]);
        	                for(int i=1; i<nticks; i++){
            		            ttickval[i]  = MathFns.truncate(ttickval[i-1]-inc, trunc[range]);
        	                }
        	                ttickval = MathFns.reverseArray(ttickval);
        	...
        	for(int i=0; i<this.nCurves; i++){
                    ...
                    if(lineOpt[i]==4){
                		...
                		for(int ii=1; ii<nxpoints; ii++){
                    			xnewpoint=xBot+(int)((((this.data[kk][ii])-(double)xLow)/xdenom)*xLen);
                    			ynewpoint=yBot-(int)((((this.data[kk+1][ii])-(double)yLow)/ydenom)*yLen);
                    			lengths[ii-1] = (int)MathFns.hypot((double)(xnewpoint-xoldpoint), (double)(ynewpoint-yoldpoint));
                		...
                		for(int ii=1; ii<nxpoints; ii++){
                    	    ...
                    	    if(Math.abs(gradients[ii-1])>0.5){
                    	        ...
                    	        for(int jj = nlpointsold+1; jj<(nlpointsnew + nlpointsold); jj++){
                    	            ...
                    	            if(MathFns.isInfinity(Math.abs(gradients[ii-1]))){
