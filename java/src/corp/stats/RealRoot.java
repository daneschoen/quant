...
public class RealRoot{
	    ...
	    while(testConv){
            ...
            if(this.iterN>this.iterMax){
                if(!this.supressLimitReachedMessage){
                    if(!this.supressNaNmessage)System.out.println("Class: RealRoot; method: brent; maximum number of iterations exceeded - root at this point, " + MathFns.truncate(mid, 4) + ", returned");
                    if(!this.supressNaNmessage)System.out.println("Last mid-point difference = " + MathFns.truncate(diff, 4) +  ", tolerance = " + this.tol);
                }
                this.root = mid;
                testConv = false;
            }
	...
	public double bisect(RealRootFunction g, double lower, double upper){
        ...
        while(testConv){
            ...
            if(this.iterN>this.iterMax){
                if(!this.supressLimitReachedMessage){
                    if(!this.supressNaNmessage)System.out.println("Class: RealRoot; method: bisect; maximum number of iterations exceeded - root at this point, " + MathFns.truncate(mid, 4) +  ", returned");
                    if(!this.supressNaNmessage)System.out.println("Last mid-point difference = " + MathFns.truncate(diff, 4) +  ", tolerance = " + this.tol);
                }
                this.root = mid;
                testConv = false;
	...
	public double falsePosition(RealRootFunction g, double lower, double upper){
        ...
        while(testConv){
            ...
            if(this.iterN>this.iterMax){
                if(!this.supressLimitReachedMessage){
                    if(!this.supressNaNmessage)System.out.println("Class: RealRoot; method: falsePostion; maximum number of iterations exceeded - root at this point, " + MathFns.truncate(mid, 4) +  ", returned");
                    if(!this.supressNaNmessage)System.out.println("Last mid-point difference = " + MathFns.truncate(diff, 4) +  ", tolerance = " + this.tol);
                }
                this.root = mid;
                testConv = false;
    ...
    // default accuracy used
   	public double bisectNewtonRaphson(RealRootDerivFunction g, double lower, double upper){
	    ...
	    while(testConv){
            ...
            if(this.iterN>this.iterMax){
                if(!this.supressLimitReachedMessage){
                    if(!this.supressNaNmessage)System.out.println("Class: RealRoot; method: bisectNewtonRaphson; maximum number of iterations exceeded - root at this point, " + MathFns.truncate(mid, 4) +  ", returned");
                    if(!this.supressNaNmessage)System.out.println("Last mid-point difference = " + MathFns.truncate(diff, 4) +  ", tolerance = " + this.tol);
                }
                this.root = mid;
                testConv = false;
	...
	public double newtonRaphson(RealRootDerivFunction g, double x){
        ...
        while(testConv){
            ...
            if(f[0]==0.0D || Math.abs(diff)<this.tol){
                ...
                x -= diff;
                f = g.function(x);
	            if(MathFns.isNaN(f[0]))throw new IllegalArgumentException("NaN returned as the function value");
	            if(MathFns.isNaN(f[1]))throw new IllegalArgumentException("NaN returned as the derivative function value");
	            if(MathFns.isNaN(f[0])){
	                if(this.returnNaN){
	                    ...
	                    throw new IllegalArgumentException("NaN as the function value");
	                }
	            }
                if(MathFns.isNaN(f[1])){
	                if(this.returnNaN){
            ...
            if(this.iterN>this.iterMax){
                if(!this.supressLimitReachedMessage){
                    if(!this.supressNaNmessage)System.out.println("Class: RealRoot; method: newtonRaphson; maximum number of iterations exceeded - root at this point, " + MathFns.truncate(x, 4) + ", returned");
                    if(!this.supressNaNmessage)System.out.println("Last mid-point difference = " + MathFns.truncate(diff, 4) +  ", tolerance = " + this.tol);
                }
                this.root = x;
                testConv = false;
    ...
    // default tolerance used
   	public static double brent(RealRootFunction g, double lower, double upper, double tol){
	    ...
	    // calculate the function value at the estimate of the lower bound of x
	    double fl = g.function(lower);
	    if(MathFns.isNaN(fl))throw new IllegalArgumentException("lower bound returned NaN as the function value");
	    if(MathFns.isNaN(fu))throw new IllegalArgumentException("upper bound returned NaN as the function value");
	    ...
	    while(testConv){
            ...
            iterN++;
            if(iterN>iterMax){
                System.out.println("RealRoot.brent; maximum number of iterations exceeded - root at this point, " + MathFns.truncate(mid, 4) +  ", returned");
                System.out.println("Last mid-point difference = " + MathFns.truncate(diff, 4) +  ", tolerance = " + tol);
                root = mid;
                testConv = false;
            }
        }
	...
	public static double bisect(RealRootFunction g, double lower, double upper, double tol){
        ...
        while(testConv){
            ...
            iterN++;
            if(iterN>iterMax){
                System.out.println("RealRoot.bisect; maximum number of iterations exceeded - root at this point, " + MathFns.truncate(mid, 4) +  ", returned");
                System.out.println("Last mid-point difference = " + MathFns.truncate(diff, 4) +  ", tolerance = " + tol);
                root = mid;
                testConv = false;
            }
	...
	public static double falsePosition(RealRootFunction g, double lower, double upper, double tol){
        ...
        while(testConv){
            ...
            iterN++;
            if(iterN>iterMax){
                System.out.println("RealRoot.falsePosition: maximum number of iterations exceeded - root at this point," + MathFns.truncate(mid, 4) +  ", returned");
                System.out.println("Last mid-point difference = " + MathFns.truncate(diff, 4) +  ", tolerance = " + tol);
                root = mid;
                testConv = false;
            }
    ...
    // tolerance supplied
   	public static double bisectNewtonRaphson(RealRootDerivFunction g, double lower, double upper, double tol){
	    ...
	    while(testConv){
            ...
            iterN++;
            if(iterN>iterMax){
                System.out.println("RealRoot.bisectNetonRaphson: maximum number of iterations exceeded - root at this point, " + MathFns.truncate(mid, 4) +  ", returned");
                System.out.println("Last mid-point difference = " + MathFns.truncate(diff, 4) +  ", tolerance = " + tol);
                root = mid;
                testConv = false;
            }
	...
	public static double newtonRaphson(RealRootDerivFunction g, double x, double tol){
        ...
        while(testConv){
            ...
            iterN++;
            if(iterN>iterMax){
                System.out.println("RealRoot.newtonRaphson: maximum number of iterations exceeded - root at this point," + MathFns.truncate(x, 4) + ", returned");
                System.out.println("Last mid-point difference = " + MathFns.truncate(diff, 4) +  ", tolerance = " + tol);
                root = x;
                testConv = false;
            }
