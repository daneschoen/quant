...
import program.graphics.PlotGraph;

//import .integration.Integration;
//import .integration.IntegralFunction;
//import .complex.*;
//import .roots.*;
...
public class StatFns{
  ...
  public static double mean(double[] aa){
    ...
    return sum/((double)n);
        ...
        public static double[] subtractMean(double[] array){
            int n = array.length;
            double mean = StatFns.mean(array);
        ...
        public static double interQuartileMean(double[] aa){
                ...
                double[] bb = MathFns.selectionSort(aa);
                double sum = 0.0D;
        ...
        public static float interQuartileMean(float[] aa){
                int n = aa.length;
                if(n<4)throw new IllegalArgumentException("At least 4 array elements needed");
                float[] bb = MathFns.selectionSort(aa);
                float sum = 0.0F;
                for(int i=n/4; i<3*n/4; i++)sum += bb[i];
       ...
       // Root mean square (rms) of a 1D array of doubles, aa
        ...
        public static double median(int[] aa){
                ...
                int nOverTwo = n/2;
                double med = 0.0D;
                int[] bb = MathFns.selectionSort(aa);
                if(MathFns.isOdd(n)){
        ...
        public static double median(long[] aa){
                ...
                int nOverTwo = n/2;
                double med = 0.0D;
                long[] bb = MathFns.selectionSort(aa);
                if(MathFns.isOdd(n)){
                    med = (double)bb[nOverTwo];
                }
                else{
        ...
        public static double volatilityLogChange(double[] array){
            ...
            return StatFns.standardDeviation(change);
        }
        ...
        public static double coefficientOfVariation(double[] array){
            return 100.0D*StatFns.standardDeviation(array)/Math.abs(StatFns.mean(array));
        }

        // Coefficient of variation of an array of float
        public static float coefficientOfVariation(float[] array){
            return 100.0F*StatFns.standardDeviation(array)/Math.abs(StatFns.mean(array));
        }
       ...
       // Variance of a 1D array of int, aa
        public static double variance(int[] aa){
                ...
                sum=0.0D;
                for(int i=0; i<n; i++){
                        sum+=MathFns.square((double)aa[i]-mean);
                }
                return sum/((double)(n-1));
        }
       ...
       // Variance of a 1D array of ilong, aa
        ...
        public static double variance(double[] aa, double[] ww){
                ...
                sumx=0.0D;
                for(int i=0; i<n; i++){
                        sumx+=weight[i]*MathFns.square(aa[i]-mean);
                }
  ...
  //* Matrix     
  public static double[][] corrCoefMatrix(double[][] matrix) {
    int dim = matrix.length;
    double[][] ccMatrix = new double[dim][dim];
    double fdCorrCoef=0.0;
    for (int i=0; i<dim; i++) {
    	for (int j=0; j<dim; j++) {
    	    if (i==j) {
    		    fdCorrCoef = 1.0;
    	    } else {
                fdCorrCoef = corrCoeff(matrix[i], matrix[j]);
    	    }
    	    ccMatrix[i][j] = fdCorrCoef;
    	}  
    }
  ...
  public static double corrCoeff(double[] xx, double[]yy){
    ...
    double s2xy = 0.0D;
    for (int i=0; i<nData; i++){
        s2xx += MathFns.square(xx[i]-mx);
        s2yy += MathFns.square(yy[i]-my);
  ...
  public static double[][] pValueMatrix(double[][] matrix, int lenX) {  
    int dim = matrix.length;
	...
	for (int i=0; i<dim; i++) {
	    for (int j=0; j<dim; j++) {
	    	if (i==j) {
	    		...
	    		//* = coeff[1] / coeffSd[1]; 
                fdTstat = matrix[i][j];  //* r value ie corrcoef
                fdTstat = fdTstat * Math.sqrt((lenX-2) / (1 - fdTstat*fdTstat));
                fdPvalue = 2*(1-studentTcdf(Math.abs(fdTstat), lenX));
	        }
	    	pMatrix[i][j] = fdPvalue;
	    }  
    }
	...
	return pMatrix;
        ...
        public static float corrCoeff(float[] x, float[] y){
            ...
            return (float)StatFns.corrCoeff(xx, yy);
        }
        ...
        public static double corrCoeff(int[] x, int[]y){
            ...
            return StatFns.corrCoeff(xx, yy);
        }
        ...
        public static double corrCoeff(double[] x, double[]y, double[] w){
            ...
            double sxy = StatFns.covariance(x, y, w);
            double sx = StatFns.variance(x, w);
            double sy = StatFns.variance(y, w);
  ...
  //* Linear correlation coefficient cumulative probability
  public static double corrCoeffProb(double rCoeff, int nu){
    ...
    double preterm = Math.exp(StatFns.logGamma((nu+1.0D)/2.0)-StatFns.logGamma(nu/2.0D))/Math.sqrt(Math.PI);
    ...
    return preterm*integral;
        ...
        public static double linearCorrCoeff(double rCoeff, int nu){
            return StatFns.corrCoeffPDF(rCoeff, nu);
        }
        ...
        public static double corrCoeffPDF(double rCoeff, int nu){
            ...
            double y = Math.pow((1.0D - MathFns.square(rCoeff)),a);
        ...
        public static double[][] histogramBins(double[] data, double binWidth, double binZero){
            ...
            for(int i=0; i<nPoints; i++){
                ...
                while(test){
                    if(j==nBins-1){
                        if(data[i]>=binWall[j] && data[i]<=binWall[j+1]*(1.0D + StatFns.histTol)){
                            binFreq[1][j]+= 1.0D;
                            dataCheck[i]=1;
                            test=false;
                        }
        ...
        public static double[][] histogramBins(double[] data, double binWidth){
            ...
            return StatFns.histogramBins(data, binWidth, binZero);
        }
        ...
        public static double[][] histogramBinsPlot(double[] data, double binWidth){
            String xLegend = null;
            return StatFns.histogramBinsPlot(data, binWidth, xLegend);
        }
        ...
        public static double[][] histogramBinsPlot(double[] data, double binWidth, String xLegend){
            ...
            return StatFns.histogramBinsPlot(data, binWidth, binZero, xLegend);
        }
        ...
        public static double gammaFunction(double x){
                ...
                if(x>=0.0){
                        if(x>=1.0D && x-(int)x==0.0D){
                         ...
                         fg = -Math.PI/(x*StatFns.gamma(-x)*Math.sin(Math.PI*x));
                }
                return fg;
        }
        ...
        public static double[] getLanczosCoeff(){
                int n = StatFns.getLanczosN()+1;
                double[] coef = new double[n];
                for(int i=0; i<n; i++){
                        coef[i] = StatFns.lgfCoeff[i];
                }
                return coef;
        }
        ...
        public static double logGammaFunction(double x){
                ...
                if(x>=0.0){
                        if(x>=1.0 && x-(int)x==0.0){
                                fg = StatFns.logFactorial(x)-Math.log(x);
                        }
                        else{
                        ...
                        fg = Math.PI/(StatFns.gamma(1.0D-x)*Math.sin(Math.PI*x));
        ...
        public static double incompleteGammaSer(double a, double x){
                ...
                double sum = 1.0/a;
                double incr = sum;
                double loggamma = StatFns.logGamma(a);
                ...
                while(check){
                        ...
                        incr *= x/a;
                        sum += incr;
                        if(Math.abs(incr) < Math.abs(sum)*StatFns.igfeps){
                                igf = sum*Math.exp(-x+acopy*Math.log(x)- loggamma);
                                check = false;
                        }
                        if(i>=StatFns.igfiter){
                                check=false;
                                igf = sum*Math.exp(-x+acopy*Math.log(x)- loggamma);
                                System.out.println("\nMaximum number of iterations were exceeded in StatFns.incompleteGammaSer().\nCurrent value returned.\nIncrement = "+String.valueOf(incr)+".\nSum = "+String.valueOf(sum)+".\nTolerance =  "+String.valueOf(igfeps));
                        }
                }
                return igf;
        }
        ...
        public static double incompleteGammaFract(double a, double x){
                ...
                while(check){
                        ...
                        if(Math.abs(first) < StatFns.FPMIN){
                            first = StatFns.FPMIN;
                        }
                        term = denom + numer/term;
                        if(Math.abs(term) < StatFns.FPMIN){
                            term = StatFns.FPMIN;
                         }
                        first = 1.0D/first;
                        incr = first*term;
                        ...
                        if(i>=StatFns.igfiter){
                                check=false;
                                System.out.println("\nMaximum number of iterations were exceeded in StatFns.incompleteGammaFract().\nCurrent value returned.\nIncrement - 1 = "+String.valueOf(incr-1)+".\nTolerance =  "+String.valueOf(igfeps));
                        }
                }
       ...
       // Returns an array of Erlang random deviates - user supplied seed
        ...
        public static double erlangMprobability(double totalTraffic, double totalResources, double em){
            ...
            if(totalTraffic>0.0D){
                ...
                double numer = totalResources*Math.log(em) - MathFns.logFactorial(em);
                double denom = 1.0D;
        ...
        public static double erlangBprobability(double totalTraffic, double totalResources){
            double prob = 0.0D;
            if(totalTraffic>0.0D){
                double numer = totalResources*Math.log(totalTraffic) - MathFns.logFactorial(totalResources);
                double denom = 1.0D;
                double lastTerm = 1.0D;
                double iCount = 1.0D;
        ...
        public static double betaCDF(double min, double max, double alpha, double beta, double limit){
            ...
            return StatFns.regularisedBetaFunction(alpha, beta, (limit-min)/(max-min));
        }
        ...
        public static double betaPDF(double min, double max, double alpha, double beta, double x){
            ...
            return pdf/StatFns.betaFunction(alpha, beta);
        }
        ...
        public static double regularisedBetaFunction(double z, double w, double x){
            ...
            if(x==0.0D){
                ...
                if(x==1.0D){
                    ...
                    ibeta = Math.exp(StatFns.logGamma(z+w) - StatFns.logGamma(z) - logGamma(w) + z*Math.log(x) + w*Math.log(1.0D-x));
                    // Continued fraction
                    if(x < (z+1.0D)/(z+w+2.0D)){
                        ibeta = ibeta*StatFns.contFract(z, w, x)/z;
                    }
                    else{
                        // Use symmetry relationship
                        ibeta = 1.0D - ibeta*StatFns.contFract(w, z, 1.0D-x)/w;
                    }
                }
            }
            return ibeta;
        ...
        public static double contFract(double a, double b, double x){
            ...
            if(Math.abs(d)<StatFns.FPMIN)d = FPMIN;
            d = 1.0D/d;
            double h = d;
            double aa = 0.0D;
            double del = 0.0D;
            ...
            while(test){
                ...
                if(Math.abs(d)<StatFns.FPMIN)d = FPMIN;
                c = 1.0D + aa/c;
                if(Math.abs(c)<StatFns.FPMIN)c = FPMIN;
                d = 1.0D/d;
                h *= d*c;
                ...
                if(i>maxit){
                    test=false;
                    System.out.println("Maximum number of iterations ("+maxit+") exceeded in StatFns.contFract in StatFns.incomplete Beta");
                }
            }
            return h;
        ...
        public static double erf(double x){
                ...
                if(x!=0.0){
                        if(x==1.0D/0.0D){
                                ...
                                if(x>=0){
                                        erf = StatFns.incompleteGamma(0.5, x*x);
                                }
                                else{
                                        erf = -StatFns.incompleteGamma(0.5, x*x);
                                }
        ...
        public static double erfc(double x){
                ...
                if(x!=0.0){
                        if(x==1.0D/0.0D){
                                ...
                                if(x>=0){
                                        erfc = 1.0D - StatFns.incompleteGamma(0.5, x*x);
                                }
                                else{
                                        erfc = 1.0D + StatFns.incompleteGamma(0.5, x*x);
                                }
        ...
        public static double normalCDF(double mean, double sd, double upperlimit){
                ...
                return (1.0D + StatFns.erf(arg))/2.0D;
        }
        // Gaussian (normal) cumulative distribution function
        // probability that a variate will assume a value less than the upperlimit
        // mean  =  the mean, sd = standard deviation
        ...
        public static double normalCDF(double mean, double sd, double lowerlimit, double upperlimit){
                ...
                return (StatFns.erf(arg2)-StatFns.erf(arg1))/2.0D;
        }
        ...
        public static double normalPDF(double mean, double sd, double x){
                return Math.exp(-MathFns.square((x - mean)/sd)/2.0)/(sd*Math.sqrt(2.0D*Math.PI));
        }
        ...
        public static double logNormalCDF(double mu, double sigma, double upperLimit){
            ...
            if(upperLimit<=0){
                    ...
                    return 0.5D*(1.0D + StatFns.erf((Math.log(upperLimit)-mu)/(sigma*Math.sqrt(2))));
                }
        }
        ...
        public static double logNormalCDF(double mu, double sigma, double lowerLimit, double upperLimit){
                ...
                if(lowerLimit!=upperLimit){
                    if(upperLimit>0.0D)arg1 = 0.5D*(1.0D + StatFns.erf((Math.log(upperLimit)-mu)/(sigma*Math.sqrt(2))));
                    if(lowerLimit>0.0D)arg2 = 0.5D*(1.0D + StatFns.erf((Math.log(lowerLimit)-mu)/(sigma*Math.sqrt(2))));
                    cdf = arg1 - arg2;
                }
        ...
        public static double logNormalPDF(double mu, double sigma, double x){
            ...
            if(x<0){
                ...
                return Math.exp(-0.5D*MathFns.square((Math.log(x)- mu)/sigma))/(x*sigma*Math.sqrt(2.0D*Math.PI));
            }
        }
        ...
        public static double[] logNormalRand(double mu, double sigma, int n){
            ...
            if(sigma<0)throw new IllegalArgumentException("The parameter sigma, " + sigma + ", must be greater than or equal to zero");
            Random rr = new Random();
            return logNormalTwoParRandGen(rr, mu, sigma, n);
        }
        ...
        // private method for generating an array of two parameter log-normal random deviates
        private static double[] logNormalTwoParRandGen(Random rr, double mu, double sigma, int n){
            double[] ran = new double[n];
            ...
            for(int i=0; i<n; i++){
                ...
                // call root searching method
                ran[i] = realR.falsePosition(logNorm2, lowerBound, upperBound);
            }
        ...
        public static double logNormalThreeParCDF(double alpha, double beta, double gamma,  double upperLimit){
            ...
            if(upperLimit<=0){
                    ...
                    return 0.5D*(1.0D + StatFns.erf(Math.log((upperLimit-alpha)/gamma)/(beta*Math.sqrt(2))));
                }
        }
        ...
        public static double logNormalThreeParCDF(double alpha, double beta, double gamma,  double lowerLimit, double upperLimit){
                ...
                if(lowerLimit!=upperLimit){
                    if(upperLimit>0.0D)arg1 = 0.5D*(1.0D + StatFns.erf(Math.log((upperLimit-alpha)/gamma)/(beta*Math.sqrt(2))));
                    if(lowerLimit>0.0D)arg2 = 0.5D*(1.0D + StatFns.erf(Math.log((lowerLimit-alpha)/gamma)/(beta*Math.sqrt(2))));
                    cdf = arg1 - arg2;
                }
        ...
        public static double logNormalThreeParPDF(double alpha, double beta, double gamma,  double x){
            ...
            if(x<0){
                ...
                return Math.exp(-0.5D*MathFns.square(Math.log((x - alpha)/gamma)/beta))/((x - gamma)*beta*Math.sqrt(2.0D*Math.PI));
            }
        }
        ...
        public static double[] logNormalThreeParRand(double alpha, double beta, double gamma,  int n){
            ...
            if(beta<0)throw new IllegalArgumentException("The parameter beta, " + beta + ", must be greater than or equal to zero");
            Random rr = new Random();
            return logNormalThreeParRandGen(rr, alpha, beta, gamma, n);
        }
        ...
        // private method for generating an array of three parameter log-normal random deviates
        private static double[] logNormalThreeParRandGen(Random rr, double alpha, double beta, double gamma,  int n){
            double[] ran = new double[n];
            ...
            for(int i=0; i<n; i++){
                ...
                // call root searching method
                ran[i] = realR.falsePosition(logNorm3, lowerBound, upperBound);
            }
        ...
        public static double logisticPDF(double mu, double beta, double x){
                return MathFns.square(MathFns.sech((x - mu)/(2.0D*beta)))/(4.0D*beta);
        }
        // Logistic probability density function
        // mu  =  location parameter, beta = scale parameter
        ...
        public static double[] logisticRand(double mu, double beta, int n){
                ...
                Random rr = new Random();
                for(int i=0; i<n; i++){
                    ran[i] = 2.0D*beta*MathFns.atanh(2.0D*rr.nextDouble() - 1.0D) + mu;
                }
                return ran;
        }
        ...
        public static double logisticStandDev(double beta){
                return Math.sqrt(MathFns.square(Math.PI*beta)/3.0D);
        }
        ...
        public static double lorentzianPDF(double mu, double gamma, double x){
                ...
                return (1.0D/Math.PI)*arg/(MathFns.square(mu-x)+arg*arg);
        }
        ...
        public static double poissonCDF(int k, double mean){
                ...
                return StatFns.incompleteGammaComplementary((double) k, mean);
        }
        ...
        public static double poissonPDF(int k, double mean){
                ...
                return Math.pow(mean, k)*Math.exp(-mean)/StatFns.factorial((double)k);
        }
                ...
                if(mean < 12.0D){
                    ...
                    for(int i=0; i<n; i++){
                        if(mean != oldm){
                            ...
                            expt = lnMean - StatFns.logGamma(mean+1.0D);
                        }
                        do{
                            do{
                            ...
                            em = Math.floor(em);
                            term = 0.9D*(1.0D+yDev*yDev)*Math.exp(em*lnMean - StatFns.logGamma(em+1.0D)-expt);
        ...
        public static double chiSquareCDF(double chiSquare, int nu){
                ...
                return StatFns.incompleteGamma((double)nu/2.0D, chiSquare/2.0D);
        }
        ...
        public static double chiSquarePDF(double chiSquare, int nu){
                ...
                return Math.pow(0.5D, dnu/2.0D)*Math.pow(chiSquare, dnu/2.0D - 1.0D)*Math.exp(-chiSquare/2.0D)/StatFns.gammaFunction(dnu/2.0D);
        }
        ...
        public static double[] chiSquareRand(int nu, int n){
            if(nu<=0)throw new IllegalArgumentException("The degrees of freedom [nu], " + nu + ", must be greater than zero");
            Random rr = new Random();
            return chiSquareRandGen(rr, nu, n);
        }
        ...
        // private method for generating an array of Chi-Square random deviates
        private static double[] chiSquareRandGen(Random rr, int nu, int n){
            double[] ran = new double[n];
            ...
            for(int i=0; i<n; i++){
                ...
                // call root searching method
                ran[i] = realR.falsePosition(csrt, lowerBound, upperBound);
            }
        ...
        public static double chiSquare(double[] observed, double[] expected, double[] variance){
            ...
            for(int i=0; i<nObs; i++){
                chi += MathFns.square(observed[i]-expected[i])/variance[i];
            }
            return chi;
        }
        ...
        public static double chiSquareFreq(double[] observedFreq, double[] expectedFreq){
            ...
            double chi = 0.0D;
            for(int i=0; i<nObs; i++){
                chi += MathFns.square(observedFreq[i]-expectedFreq[i])/expectedFreq[i];
            }
            return chi;
        }
        ...
        public static double binomialCDF(double p, int n, int k){
                ...
                return StatFns.regularisedBetaFunction(k, n-k+1, p);
        }
        // Returns the binomial cumulative probabilty
        ...
        public static double binomialPDF(double p, int n, int k){
                if(k<0 || n<0)throw new IllegalArgumentException("\nn and k must be greater than or equal to zero");
                if(k>n)throw new IllegalArgumentException("\nk is greater than n");
                return Math.floor(0.5D + Math.exp(StatFns.logFactorial(n) - StatFns.logFactorial(k) - StatFns.logFactorial(n-k)))*Math.pow(p, k)*Math.pow(1.0D - p, n - k);
        }
        ...
        public static double binomialCoeff(int n, int k){
                if(k<0 || n<0)throw new IllegalArgumentException("\nn and k must be greater than or equal to zero");
                if(k>n)throw new IllegalArgumentException("\nk is greater than n");
                return Math.floor(0.5D + Math.exp(StatFns.logFactorial(n) - StatFns.logFactorial(k) - StatFns.logFactorial(n-k)));
        }
            ...
            for(int i=0; i<n; i++){
	            ...
	            if(nTrials < 25) {
		            ...
		            if(nTrials != nOld) {
			            ...
			            oldGamma = StatFns.logGamma(enTrials + 1.0D);
			            nOld = nTrials;
		            }
		            ...
		            do{
			            ...
			            testDeviate = 1.2D*sq*(1.0D + tanW*tanW)*Math.exp(oldGamma - StatFns.logGamma(hold0 + 1.0D) - StatFns.logGamma(enTrials - hold0 + 1.0D) + hold0*logProb + (enTrials - hold0)*logProbC);
  ...
  public static double fCompCDF(double fValue, int df1, int df2){
    ...
    return StatFns.regularisedBetaFunction(df2/2.0D, df1/2.0D, x);
  }
  ...
  public static double fCompCDF(double var1, int df1, double var2, int df2){
    ...
    return StatFns.regularisedBetaFunction(df2/2.0D, df1/2.0D, x);
        ...
        public static double fTestValueGivenFprob(double fProb, int df1, int df2){
            ...
            double diff = (MathFns.log10(fTestValues[fTestsNum-1])-MathFns.log10(fTestValues[0]))/(fTestsNum-1);
            // Fill array
            for(int i=1; i<fTestsNum-1; i++){
                fTestValues[i] = Math.pow(10.0D,(MathFns.log10(fTestValues[i-1])+diff));
            }
            ...
            for(int i=0; i<fTestsNum; i++){
                fTestProb[i] = StatFns.fCompCDF(fTestValues[i], df1, df2);
            }
        ...
        private static double fTestBisect(double fProb, double fTestLow, double fTestHigh, int df1, int df2, int endTest){
            ...
            double funcLow = fProb - StatFns.fCompCDF(fTestLow, df1, df2);
            double funcHigh = fProb - StatFns.fCompCDF(fTestHigh, df1, df2);
            ...
            while(test0){
                if(funcLow*funcHigh>0.0D){
                    if(endTest<0){
                        ...
                        fTestLow /= 10.0D;
                        funcLow = fProb - StatFns.fCompCDF(fTestLow, df1, df2);
                    }
                    else{
                        nExtensions++;
                        if(nExtensions>100){
                        ...
                        fTestHigh *= 10.0D;
                        funcHigh = fProb - StatFns.fCompCDF(fTestHigh, df1, df2);
                    }
                }
                else{
                    test0=false;
                ...
                while(test1){
                    ...
                    funcMid = fProb - StatFns.fCompCDF(fTestMid, df1, df2);
        ...
        public double fPDF(double fValue, int nu1, int nu2){
            ...
            double denom = fValue*StatFns.betaFunction(dnu1/2.0D, dnu2/2.0D);
            return numer/denom;
        }
  ...
  // generate an array of F-distribution random deviates
  private static double[] fRandGen(Random rr, int nu1, int nu2, int n){
    double[] ran = new double[n];
    ...
    for (int i=0; i<n; i++){
        ...
        realR.supressLimitReachedMessage();
        // call root searching method
        ran[i] = realR.falsePosition(frt, lowerBound, upperBound);
    }
  ...
  public static double studentT(double tValue, int df){
    ...
    return ((StatFns.gamma(dfterm)/StatFns.gamma(ddf/2))/Math.sqrt(ddf*Math.PI))*Math.pow(1.0D + tValue*tValue/ddf, -dfterm);
  }
  ...
  public static double studentTPDF(double tValue, int df){
    ...
    return ((StatFns.gamma(dfterm)/StatFns.gamma(ddf/2))/Math.sqrt(ddf*Math.PI))*Math.pow(1.0D + tValue*tValue/ddf, -dfterm);
        ...
        public static double studentTProb(double tValue, int df){
            ...
            return 0.5D*(1.0D + (StatFns.regularisedBetaFunction(ddf/2.0D, 0.5D, 1) - StatFns.regularisedBetaFunction(ddf/2.0D, 0.5D, x))*MathFns.sign(tValue));
        }
...
*/
        ...
        public static double probAtn(double tValue, int df){
            double ddf = (double)df;
            double x = ddf/(ddf+tValue*tValue);
            return 1.0D - StatFns.regularisedBetaFunction(ddf/2.0D, 0.5D, x);
        }

   // Returns an array of Student's t random deviates - clock seed, nu is degrees of freedom
   ...
   // Returns an array of a Student's t random deviates - user supplied seed, nu is degrees of freedom
  ...
  // Generate an array of student's T random deviates
  private static double[] studentTrandGen(Random rr, int nu, int n){
    ...
    for (int i=0; i<n; i++) {
        ...
        realR.supressLimitReachedMessage();
        // call root searching method
        rnd[i] = realR.falsePosition(strt, lowerBound, upperBound);
    }
    ...
    return rnd;
       ...
       // Returns an array of maximal Gumbel (Type I EVD) random deviates - user supplied seed
        ...
        public static double gumbelMinMean(double mu,double sigma){
                return mu - sigma*MathFns.EULER_CONSTANT_GAMMA;
        }
        ...
        public static double frechetMean(double mu,double sigma, double gamma){
                double y = Double.NaN;
                if(gamma>1.0D){
                    y = mu + sigma*StatFns.gamma(1.0D-1.0D/gamma);
                }
                return y;
        }
        ...
        public static double frechetStandDev(double sigma, double gamma){
                double y = Double.NaN;
                if(gamma>2.0D){
                    y = StatFns.gamma(1.0D-2.0D/gamma)-MathFns.square(StatFns.gamma(1.0D-1.0D/gamma));
                    y = sigma*Math.sqrt(y);
                }
                return y;
        }
        ...
        public static double weibullMean(double mu,double sigma, double gamma){
                return mu + sigma*StatFns.gamma(1.0D/gamma+1.0D);
        }

        // Weibull standard deviation
        public static double weibullStandDev(double sigma, double gamma){
                double y = StatFns.gamma(2.0D/gamma+1.0D)-MathFns.square(StatFns.gamma(1.0D/gamma+1.0D));
        ...
        public static double paretoStandDev(double alpha, double beta){
                double y = Double.NaN;
                if(alpha>1.0D)y = alpha*MathFns.square(beta)/(MathFns.square(alpha-1)*(alpha-2));
                return y;
        }
        ...
        public static Vector<Object> upperOutliersAnscombe(double[] values, double constant){
            ...
            while(test){
                ...
                double mean = StatFns.mean(copy0);
                double standDev = StatFns.standardDeviation(copy0);
                double max = MathFns.maximum(copy0);
                ...
                for(int j=0; j<nOutliers; j++){
                    ...
                    while(testO){
                        if(outliers[j]==values[i] && indCheck[i]){
                            ...
                            if(i>=nValues0){
                                testO=false;
                                System.out.println("StatFns.outliersUpperAnscombe");
        ...
        public static Vector<Object> lowerOutliersAnscombe(double[] values, double constant){
            ...
            while(test){
                ...
                double mean = StatFns.mean(copy0);
                double standDev = StatFns.standardDeviation(copy0);
                double min = MathFns.minimum(copy0);
...
class StudentTfunct implements RealRootFunction{
        ...
        public double function(double x){
            ...
            double y = cfd - StatFns.studentTcdf(x, nu);
...
class GammaFunct implements RealRootFunction{
        ...
        public double function(double x){
            ...
            double y = cfd - StatFns.gammaCDF(mu, beta, gamma, x);
...
class BetaFunct implements RealRootFunction{
        ...
        public double function(double x){
            ...
            double y = cfd - StatFns.betaCDF(min, max, alpha, beta, x);
...
class ErlangBfunct implements RealRootFunction{
         ...
         public double function(double x){
                return blockingProbability - StatFns.erlangBprobability(x, totalResources);
         }
}
...
class ErlangCfunct implements RealRootFunction{
         ...
         public double function(double x){
                return nonZeroDelayProbability - StatFns.erlangCprobability(x, totalResources);
         }
}
...
class EngsetProb implements RealRootFunction{
         ...
         public double function(double x){
                ...
                double pNumer = StatFns.logFactorial(numberOfSources-1) - StatFns.logFactorial(totalResources) - StatFns.logFactorial(numberOfSources-1-totalResources);
                double pDenom = 0.0D;
                double iDenom = 0.0D;
                double iCount = 0.0D;
                double pTerm = 0.0D;
                while(iCount<=totalResources){
                    iDenom = StatFns.logFactorial(numberOfSources-1) - StatFns.logFactorial(iCount) - StatFns.logFactorial(numberOfSources-1-iCount);
...
class EngsetLoad implements RealRootFunction{
         ...
         public double function(double x){
                return blockingProbability - StatFns.engsetProbability(x, totalResources, numberOfSources);
         }
}
...
class ChiSquareFunct implements RealRootFunction{
        ...
        public double function(double x){
            ...
            double y = cfd - StatFns.chiSquareCDF(x, nu);
...
class fFunct implements RealRootFunction{
        ...
        public double function(double x){
            ...
            double y = cfd - (1.0D - StatFns.fCompCDF(x, nu1, nu2));
...
class LogNormalTwoParFunct implements RealRootFunction{
        ...
        public double function(double x){
            ...
            double y = cfd - StatFns.logNormalCDF(mu, sigma, x);
...
class LogNormalThreeParFunct implements RealRootFunction{
        ...
        public double function(double x){
            ...
            double y = cfd - StatFns.logNormalThreeParCDF(alpha, beta, gamma, x);
