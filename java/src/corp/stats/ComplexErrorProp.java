...
/*********************************************************/
        ...
        public static double calcRho(double[] x, double[] y){
                ...
                double covarXY = 0.0D;
                for(int i=0; i<n; i++){
                   varX+= MathFns.square(x[i]-meanX);
                   varY += MathFns.square(y[i]-meanY);
        ...
        public static ComplexErrorProp over(ComplexErrorProp a, ComplexErrorProp b){
                ...
                for(int i=0; i<ComplexErrorProp.monteCarloLength; i++){
                    cRealErrorSum += MathFns.square(cRealSum - cReal[i]);
                    cImagErrorSum += MathFns.square(cImagSum - cImag[i]);
                }
        ...
        public static ComplexErrorProp truncate(ComplexErrorProp x, int prec){
                ...
                rV = MathFns.truncate(rV, prec);
                rE = MathFns.truncate(rE, prec);
                iV = MathFns.truncate(iV, prec);
                iE = MathFns.truncate(iE, prec);
