...
public class MathFns{
        ...
        public static double maximumDifference(double[] aa){
            return MathFns.maximum(aa) - MathFns.minimum(aa);
        }
        ...
        public static double minimumDifference(double[] aa){
            double[] sorted = MathFns.selectionSort(aa);
            double n = aa.length;
        ...
        public static float minimumDifference(float[] aa){
            float[] sorted = MathFns.selectionSort(aa);
            float n = aa.length;
        ...
        public static long minimumDifference(long[] aa){
            long[] sorted = MathFns.selectionSort(aa);
            long n = aa.length;
        ...
        public static int minimumDifference(int[] aa){
            int[] sorted = MathFns.selectionSort(aa);
            int n = aa.length;
        ...
        public static int[] indicesOf(double[] array, double value){
            ...
            if(numberOfIndices!=0){
                for(int i=0; i<numberOfIndices; i++){
                    indices[i] = ((Integer)vec.elementAt(i)).intValue();
                }
            }
            return indices;
        }
       ...
       public static void selectionSort(int[] aa, double[] bb, int[] cc, double[] dd){
        ...
        public static double calcBMIimperial(double height, double weight){
                height = MathFns.footToMetre(height);
                weight = MathFns.poundToKg(weight);
            return  weight/(height*height);
        }
        ...
        public static double calcWeightFromBMIimperial(double bmi, double height){
            ...
            weight = MathFns.kgToPound(weight);
            return  weight;
        }
        ...
        public static double angle(double xAtA, double yAtA, double xAtB, double yAtB, double xAtC, double yAtC){
            ...
            double ccos = MathFns.cos(xAtA, yAtA, xAtB, yAtB, xAtC, yAtC);
        ...
        public static double angle(double sideAC, double sideBC, double sideAB){
            ...
            double ccos = MathFns.cos(sideAC, sideBC, sideAB);
            return Math.acos(ccos);
        }
        ...
        public static double sin(double xAtA, double yAtA, double xAtB, double yAtB, double xAtC, double yAtC){
            double angle = MathFns.angle(xAtA, yAtA, xAtB, yAtB, xAtC, yAtC);
        ...
        public static double sin(double sideAC, double sideBC, double sideAB){
            double angle = MathFns.angle(sideAC, sideBC, sideAB);
            return Math.sin(angle);
        }
        ...
        // Inverse sine
        // MathFns.asin Checks limits - Java Math.asin returns NaN if without limits
        public static double asin(double a){
            if(a<-1.0D && a>1.0D) throw new IllegalArgumentException("MathFns.asin argument (" + a + ") must be >= -1.0 and <= 1.0");
            return Math.asin(a);
        }
        ...
        public static double cos(double xAtA, double yAtA, double xAtB, double yAtB, double xAtC, double yAtC){
            double sideAC = MathFns.hypot(xAtA - xAtC, yAtA - yAtC);
            double sideBC = MathFns.hypot(xAtB - xAtC, yAtB - yAtC);
            double sideAB = MathFns.hypot(xAtA - xAtB, yAtA - yAtB);
            return MathFns.cos(sideAC, sideBC, sideAB);
        }
        ...
        public static double acos(double a){
            if(a<-1.0D || a>1.0D) throw new IllegalArgumentException("MathFns.acos argument (" + a + ") must be >= -1.0 and <= 1.0");
            return Math.acos(a);
        }
    ...
    public static double truncate(double xDouble, int trunc){
        ...
        if(!MathFns.isNaN(xDouble)){
            if(!MathFns.isPlusInfinity(xDouble)){
                if(!MathFns.isMinusInfinity(xDouble)){
                    if(xDouble!=0.0D){
    ...
    public static float truncate(float xFloat, int trunc){
        ...
        if(!MathFns.isNaN(xFloat)){
            if(!MathFns.isPlusInfinity(xFloat)){
                if(!MathFns.isMinusInfinity(xFloat)){
                    if(xFloat!=0.0D){
    ...
    private static String truncateProcedure(String xValue, int trunc){
        ...
        public static boolean isEqual(double x, double y){
            boolean test=false;
            if(MathFns.isNaN(x)){
                if(MathFns.isNaN(y))test=true;
            }
            else{
                if(MathFns.isPlusInfinity(x)){
                    if(MathFns.isPlusInfinity(y))test=true;
                }
                else{
                    if(MathFns.isMinusInfinity(x)){
                        if(MathFns.isMinusInfinity(y))test=true;
                    }
                    else{
                        if(x==y)test=true;
                    }
        ...
        public static long dateToJavaMilliS(int year, int month, int day, int hour, int min, int sec){
            ...
            while(yearTest>=1970){
                yearDiff += 365;
                if(MathFns.leapYear(yearTest))yearDiff++;
                yearTest--;
            }
            ...
            while(monthTest>0){
                ...
                if(MathFns.leapYear(year))monthDiff++;
                monthTest--;
            }
